...
let allServerData = [];
let filteredData_storageDetail = [];
let globalStorageDetailData = [];
let currentStorageDetailPage = 1;
let storageDetailItemsPerPage = 100;
let totalStorageDetailCount = 0;
let totalStorageDetailPages = 0;
let totalQty = 0;
let currentSortColumn = null;
let currentSortOrder = 'asc';

$(document).ready(function() {
	window.filteredStorageDetailData = [];
	...
	// 메인 호출 함수 - 메뉴 클릭 시 호출
	window.call_mPurchase_storage_detail = function(menuId) {
		showLoading("data");
		...
		performStorageDetailDBSearch({ fromDate, toDate, storage1, storage2 });
	};
	...
	function performStorageDetailDBSearch(searchCriteria) {
		...
		$.ajax({
			...
			success: function(response) {
				...
				filteredData_storageDetail = [...allServerData];
				...
				// 첫 번째 검색이라면 뷰를 렌더링
				if (!$('#view_mPurchase_storage_detail').length) {
	...
	function applyClientPagination() {
		storageDetailItemsPerPage = parseInt(getCookie('itemsPerPage')) || 100;

		totalStorageDetailCount = filteredData_storageDetail.length;
		totalStorageDetailPages = Math.ceil(totalStorageDetailCount / storageDetailItemsPerPage);

		const startIndex = (currentStorageDetailPage - 1) * storageDetailItemsPerPage;
		const endIndex = startIndex + storageDetailItemsPerPage;

		globalStorageDetailData = filteredData_storageDetail.slice(startIndex, endIndex);
	...
	function applyClientSort(column, dataType) {
		...
		filteredData_storageDetail.sort((a, b) => {
	...
	function renderStorageDetailView() {
		...
		if (loginid == "wms") {
			btnHtml = `
	            <input type="button" value="관리자용 삭제" class="btn btn-danger btnAdminStorageDetailDelete"/>
	        `;
			intfBtnHtml = `				
				<div class="btnInterfaceCommon btnStorageDetailItemsArea" style="margin-left:24px;">
					<input type="button" value="${i18n.t('btn.Intf')}" class="btn btn-success btnIntfStorageDetail"/>
					<input type="button" value="${i18n.t('btn.Intf.delete')}" class="btn btn-warning btnIntfStorageDetailDelete"/>
				</div>
			`;
		}

		let content_output = `
			<div class="divBlockControl" id="view_mPurchase_storage_detail">
					...
					<div class="search-area">
						<div class="search-row">
							...
							<div class="search-label">
								...
								<input type="text" id="storageDetail_searchVal_oitemcode" />
							</div>
							<div class="search-label">
						...
						<div class="search_button_area">
							<button class="btn btn-primary btnStorageDetailSearch">${i18n.t('btn.search')}</button>
							<button class="btn btn-secondary btnStorageDetailSearchInit">${i18n.t('btn.clear')}</button>
						</div>
					</div>
					...
					<div class="table-container">
						...
						<div class="table-info">
							<span>${i18n.t('table.info.total')} <strong id="storageDetailTotalCount">${totalStorageDetailCount}</strong> ${i18n.t('table.info.records')} | 
								...
								<span class="tqtyTitle">${i18n.t('table.info.qty')} : </span><span class="storageDetailTotalQty" style="color:#007bff"></span> 
							</span>
							<div class="action-buttons-right mPurchase_storage_detail">
								<div id="defaultActions" class="action-group">
									${btnHtml}
									<input type="button" value="${i18n.t('btn.delete')}" class="btn btn-danger btnStorageDetailDelete"/>
									<button class="btn btn-success" id="storageDetailExcelBtn" onclick="downloadAllStorageDetailData()">Excel</button>
								</div>
							</div>
							${intfBtnHtml}
						</div>
						<table class="data-table mPurchase_storage_detail" id="storageDetailTable">
							<thead>
								<tr>
									<th class = "checkboxVal">
										<input type="checkbox" class="storageDetail_chkAll">
									</th>
									...
									<th class = "cnameVal" data-sort="OITEMCODE">${i18n.t('search.oitemcode')}<!-- OITEMCODE --></th>
							...
							<tbody id="storageDetailDetailTableBody">
							</tbody>
						</table>
						...
						<div class="items-per-page-selector">
					        <label for="storageDetail_itemsPerPage">${i18n.t('table.itemsPerPage')}:</label>
					        <select id="storageDetail_itemsPerPage" class="items-per-page-select">
		...
		$("#storageDetail_itemsPerPage").val(storageDetailItemsPerPage);
	...
	function updateStorageDetailTotalCount() {
		...
		$('#storageDetailTotalPageInfo').text(totalStorageDetailPages);
	}
	...
	function renderStorageDetailTableData() {
		...
		for (let i = 0; i < globalStorageDetailData.length; i++) {
			...
			tableBody += `
				<tr>
		            <td class = "checkboxVal"><input type="checkbox" class="storageDetail_chk ${statusClass}" 
		    			data-unique="${data.SDATE}|${data.ITEMCODE}|${data.INTF_YN}|${data.QTY}|${data.FACTORY}|${data.STORAGE2}|${data.STORAGE1}|${data.BARCODE}|${data.MES_KEY}"
		    			data-delete="${data.IID}|${data.SDATE}|${data.FACTORY}|${data.STORAGE2}|${data.BARCODE}">
		    		</td>
		...
		$("#storageDetailDetailTableBody").html(tableBody);
		$(".storageDetail_chkAll").prop("checked", false);
	}
	...
	function renderStorageDetailPagination() {
		...
		let endPage = Math.min(totalStorageDetailPages, currentStorageDetailPage + 5);
		...
		if (endPage < totalStorageDetailPages) {
			if (endPage < totalStorageDetailPages - 1) {
				paginationHtml += `<span class="page-dots">...</span>`;
			}
			paginationHtml += `<button class="storageDetail-page-btn" data-page="${totalStorageDetailPages}">${totalStorageDetailPages}</button>`;
		}

		if (currentStorageDetailPage < totalStorageDetailPages) {
			paginationHtml += `<button class="storageDetail-page-btn" data-page="${currentStorageDetailPage + 1}">&gt;</button>`;
		} else {
			paginationHtml += `<button class="storageDetail-page-btn disabled">&gt;</button>`;
		}
	...
	function bindStorageDetailEvents() {
		// 전체 선택 체크박스
		$(document).off('change', '.storageDetail_chkAll').on('change', '.storageDetail_chkAll', function() {
			let isChecked = $(this).is(':checked');
			$('.storageDetail_chk').prop('checked', isChecked);
		});

		// 개별 체크박스
		$(document).off('change', '.storageDetail_chk').on('change', '.storageDetail_chk', function() {
			let totalCheckboxes = $('.storageDetail_chk').length;
			let checkedCheckboxes = $('.storageDetail_chk:checked').length;
			$('.storageDetail_chkAll').prop('checked', totalCheckboxes === checkedCheckboxes);
		});
		...
		$('#storageDetail_itemsPerPage').off('change').on('change', function() {
			const newItemsPerPage = parseInt($(this).val());
			changeStorageDetailItemsPerPage(newItemsPerPage);
		});
		...
		$('#storageDetailTable thead th[data-sort]').off('click').on('click', function() {
			const column = $(this).data('sort');
			const dataType = $(this).data('type') || 'string';
			applyClientSort(column, dataType);
		});

		$('#view_mPurchase_storage_detail input[type="text"], #view_mPurchase_storage_detail input[type="date"]').off('keypress').on('keypress', function(e) {
			if (e.which === 13) {
				performStorageDetailSearch();
			}
		});
	...
	function getCurrentSearchCriteria() {
		return {
			...
			oitemcode: $("#storageDetail_searchVal_oitemcode").val().trim().toUpperCase(),
	...
	function resetStorageDetailSearch() {
		...
		$("#storageDetail_searchVal_oitemcode").val('');
	...
	function updateTotalQty() {
		$(".storageDetailTotalQty").text(Number(totalQty).toLocaleString());
	}
	...
	window.exportStorageDetailData = function() {
		return {
			total: filteredData_storageDetail.length,
			currentPage: currentStorageDetailPage,
			itemsPerPage: storageDetailItemsPerPage,
			data: filteredData_storageDetail
		};
	}

	//삭제
	$(document).off("click", ".btnStorageDetailDelete").on("click", ".btnStorageDetailDelete", function() {
		const $btn = $(this);
		if ($btn.prop('disabled')) return;

		let checkedLength = $(".storageDetail_chk:checked").length;
		const iidList = [];
		...
		$(".storageDetail_chk:checked").each(function() {
	...
	// 관리자용 삭제
	$(document).off("click", ".btnAdminStorageDetailDelete").on("click", ".btnAdminStorageDetailDelete", function() {
		let checkedLength = $(".storageDetail_chk:checked").length;
		const iidList = [];
	...
	//인터페이스 등록
	$(document).off("click", ".btnIntfStorageDetail").on("click", ".btnIntfStorageDetail", function() {

		if ($(".storageDetail_chk.status-completed:checked").length > 0) {
	...
	//인터페이스 등록 취소
	$(document).off("click", ".btnIntfStorageDetailDelete").on("click", ".btnIntfStorageDetailDelete", function() {

		if ($(".storageDetail_chk.status-waiting:checked").length > 0) {
...
window.downloadAllStorageDetailData = function() {
	...
	const processedData = filteredData_storageDetail.map(item => {
		return {
			...item,
	...
	ExcelExporter.downloadExcel(processedData, window.storageDetailColumns, {
