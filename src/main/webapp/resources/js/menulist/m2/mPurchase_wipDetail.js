...
let allServerData = [];
let filteredData_wipDetail = [];
let globalWipDetailData = [];
let currentWipDetailPage = 1;
let wipDetailItemsPerPage = 100;
let totalWipDetailCount = 0;
let totalWipDetailPages = 0;
let totalQty = 0;
...
$(document).ready(function() {
	...
	window.call_mPurchase_wipDetail = function(menuId) {
		showLoading("data");
		const { fromDate, toDate } = getDefaultDateRange();
		const factory = getCookie('selectedFactory');

		performWipDetailDBSearch({ factory, toDate, fromDate });
	}
...
function performWipDetailDBSearch(searchCriteria) {
	...
	$.ajax({
		...
		success: function(response) {
			...
			filteredData_wipDetail = [...allServerData];
			...
			// 첫 번째 검색이라면 뷰를 렌더링
			if (!$('#view_mPurchase_wipDetail').length) {
				renderWipDetailView();
			} else {
...
function applyClientPagination() {
	wipDetailItemsPerPage = parseInt(getCookie('itemsPerPage')) || 100;

	totalWipDetailCount = filteredData_wipDetail.length;
	totalWipDetailPages = Math.ceil(totalWipDetailCount / wipDetailItemsPerPage);

	const startIndex = (currentWipDetailPage - 1) * wipDetailItemsPerPage;
	const endIndex = startIndex + wipDetailItemsPerPage;

	globalWipDetailData = filteredData_wipDetail.slice(startIndex, endIndex);
...
function applyClientSort(column, dataType) {
	...
	filteredData_wipDetail.sort((a, b) => {
...
function renderWipDetailView() {
	...
	if (loginid == "wms" || loginid == "master") {
		...
		<th class = 'intfVal' data-sort="WMS_KEY">${i18n.t('table.ifno')}<!-- 인터페이스 --></th>
		`
	} else {
	...
	if (loginid == "wms") {
		btnHtml = `
            <input type="button" value="관리자용 삭제" class="btn btn-danger btnAdminWipDelete"/>
        `;
	}

	let content_output = `
			<div class="divBlockControl" id="view_mPurchase_wipDetail">
				<div class="content-body">
					...
					<div class="table-container">
						...
						<div class="table-info">
							<span>${i18n.t('table.info.total')} <strong id="wipDetailTotalCount">${totalWipDetailCount}</strong> ${i18n.t('table.info.records')} | 
								${i18n.t('table.page')} <strong id="wipDetailCurrentPageInfo">${currentWipDetailPage}</strong>/<strong id="wipDetailTotalPageInfo">${totalWipDetailPages}</strong> |  
								<span class="tqtyTitle">${i18n.t('table.info.qty')} : </span><span class="wipDetailTotalQty" style="color:#007bff"></span> 
							</span>
							<div class="action-buttons-right mPurchase_wipDetail">
								<div id="defaultActions" class="action-group">
									${btnHtml}
									<input type="button" value="${i18n.t('btn.delete')}" class="btn btn-danger btnWipDelete" style="display:none"/>
									<button class="btn btn-success" id="wipDetailExcelBtn" onclick="downloadAllWipDetailData()">Excel</button>
								</div>
							</div>
							${btnAreaHtml}
						</div>
						<table class="data-table mPurchase_wipDetail" id="wipDetailTable">
							<thead>
								<tr>
							        ${nohtml}		
						...
						<div class="items-per-page-selector">
					        <label for="wipDetail_itemsPerPage">${i18n.t('table.itemsPerPage')}:</label>
					        <select id="wipDetail_itemsPerPage" class="items-per-page-select">
	...
	$("#wipDetail_itemsPerPage").val(wipDetailItemsPerPage);
...
function renderFactoryStorage() {
	...
	const wccode = $('#wipDetail_searchVal_wccode'); // ✅ 추가
...
function updateWipDetailTotalCount() {
	$('#wipDetailTotalCount').text(Number(totalWipDetailCount).toLocaleString());
	$('#wipDetailCurrentPageInfo').text(currentWipDetailPage);
	$('#wipDetailTotalPageInfo').text(totalWipDetailPages);
}
...
function renderWipDetailTableData() {
	...
	for (let i = 0; i < globalWipDetailData.length; i++) {
		//console.log(globalWipDetailData[i]);
		...
		tableBody += `
			<tr>
				...
				<td class = "loginidVal">${globalWipDetailData[i].loginid || globalWipDetailData[i].LOGINID || ''}</td>
...
function renderWipDetailPagination() {
	...
	let endPage = Math.min(totalWipDetailPages, currentWipDetailPage + 5);
	...
	if (endPage < totalWipDetailPages) {
		if (endPage < totalWipDetailPages - 1) {
			paginationHtml += `<span class="page-dots">...</span>`;
		}
		paginationHtml += `<button class="wipDetail-page-btn" data-page="${totalWipDetailPages}">${totalWipDetailPages}</button>`;
	}

	if (currentWipDetailPage < totalWipDetailPages) {
		paginationHtml += `<button class="wipDetail-page-btn" data-page="${currentWipDetailPage + 1}">&gt;</button>`;
	} else {
		paginationHtml += `<button class="wipDetail-page-btn disabled">&gt;</button>`;
	}
...
function bindWipDetailEvents() {
	...
	$(document).off('change', '.wipDetail_chkAll').on('change', '.wipDetail_chkAll', function() {
		...
		$('.wipDetail_chk:not(:disabled)').prop('checked', isChecked);
	});

	// ✅ 개별 체크박스 (추가)
	$(document).off('change', '.wipDetail_chk').on('change', '.wipDetail_chk', function() {
		const totalCheckboxes = $('.wipDetail_chk:not(:disabled)').length;
		const checkedCheckboxes = $('.wipDetail_chk:checked:not(:disabled)').length;
	...
	$('#wipDetail_itemsPerPage').off('change').on('change', function() {
		const newItemsPerPage = parseInt($(this).val());
		changeWipDetailItemsPerPage(newItemsPerPage);
	});
	...
	$('#wipDetailTable thead th[data-sort]').off('click').on('click', function() {
		const column = $(this).data('sort');
		const dataType = $(this).data('type') || 'string';
		applyClientSort(column, dataType);
	});

	$('#view_mPurchase_wipDetail input[type="text"], #view_mPurchase_wipDetail input[type="date"]').off('keypress').on('keypress', function(e) {
		if (e.which === 13) {
			performWipDetailSearch();
		}
	});
...
function resetWipDetailSearch() {
	...
	currentWipDetailPage = 1;
	performWipDetailDBSearch({ factory, storage: 'Material', toDate, fromDate });

	console.log('검색 조건이 초기화되었습니다.');
}

function updateTotalQty() {
	$(".wipDetailTotalQty").text(Number(totalQty).toLocaleString());
}
...
window.downloadAllWipDetailData = function() {
	...
	ExcelExporter.downloadExcel(filteredData_wipDetail, window.wipDetailColumns, {
...
// 관리자용 삭제
$(document).on("click", ".btnAdminWipDelete", function() {
	if ($(".wipDetail_chk.status-completed:checked").length > 0) {
		alert(i18n.t('validation.confirm.items.delete'));
		// 확정된 항목이 있습니다. 인퍼테이스 삭제 후 항목을 삭제할 수 있습니다.
		return;
...
function updateCheckboxStatus() {
	let totalCheckboxes = $('.wipDetail_chk:not(:disabled)').length;
	let checkedCheckboxes = $('.wipDetail_chk:checked:not(:disabled)').length;

	if (checkedCheckboxes === 0) {
		$('.wipDetail_chkAll').prop('indeterminate', false);
		$('.wipDetail_chkAll').prop('checked', false);
	} else if (checkedCheckboxes === totalCheckboxes) {
		$('.wipDetail_chkAll').prop('indeterminate', false);
		$('.wipDetail_chkAll').prop('checked', true);
	} else {
		$('.wipDetail_chkAll').prop('indeterminate', true);
	}
}
