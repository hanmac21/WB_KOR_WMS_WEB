...
$(document).ready(function() {
	...
	window.loadRackListData = function() {
		return new Promise((resolve, reject) => {
			...
			let storage = $('#rack-filter-storageVal').val() || 'default';
	...
	window.generateStructure = function(rackId, rackData) {
		...
		for (let i = 1; i <= moduleCount; i++) {
			...
			// ⭐ 모듈 데이터가 없어도 빈 객체로 처리
			const moduleData = rackData?.modules?.[num - 1] || { positions: [] };
			...
			if ($("#rack-filter-factoryVal").val() == 'WBTA') {
				...
				['D', 'C', 'B', 'A'].forEach(level => {
	                ...
	                `;
						...
						if (status === 'occupied') {
							// DB에서 받은 실제 차량 정보 사용
							html += `<div class="car-info">${positionData.qty || 'N/A'}<br>${positionData.itemcode || 'N/A'}<br></div>`;
				...
				['4', '3', '2', '1'].forEach(level => {
					...
					for (let pos2 = 1; pos2 <= 2; pos2++) {
						...
						const positionId = `${rackId}-${num}-${level}-${pos}`;

						// 🆕 **DB 연동 6**: DB에서 해당 포지션의 실제 데이터 가져오기 //개선
						const positionData = moduleData?.positions?.find(p =>
							p.positionId === positionId ||
							(p.module == num && p.level == level && p.position == pos)
						) || {};
						console.log(" LOCA INFO - POS ")
						console.log(positionData)
						console.log(moduleData)
		...
		list.forEach(function(data) {
			...
			infoHtml += `
					<div class="modal-data">							
						...
						<div class="modal-info">
							<div class="info-label">${i18n.t('search.oitemcode')}</div>
	...
	function initFactoryStorageFilter($root) {
		...
		function updatePart2(siteRaw) {
			...
			if (savedStorage === 'ILLINOIS') {
				storage.innerHTML = '';
				storage.add(new Option('OUTSIDE', 'OUTSIDE'));
				storage.value = 'OUTSIDE';
	...
	window.changeWarehouseImg = function() {
		...
		if ($("#rack-filter-factoryVal").val() == 'WBTA') {
			...
			} else if ($("#rack-filter-storageVal").val() == 'Side seat') {
				$("#warehouse-display").append('<img src="../resources/images/sideseatmap.png" style = "width:100%">')
			} else {
				$("#warehouse-display").append('<img src="../resources/images/woobo_wms_main_img.png" style = "width:100%">')
			}
		} else {
			$("#warehouse-display").append('<img src="../resources/images/woobo_wms_main_img.png" style = "width:87%">')
		}
	}
	...
	window.updateRackList = function(rackListData) {
		...
		$('.rack-filter-resultArea').html(`
			...
			<span style="font-size: 16pt; opacity: 0.8;">TOTAL : ${totalCapacity}</span>
		`);
	}
	        ...
	        <div class="divBlockControl" id="view_m2_5_2">
	            ...
	            <div class="structure-area">
	                ...
	                <div class="warehouse-structure" id="warehouse-display">
						<img src="../resources/images/woobo_wms_main_img.png" style = "width:100%">
...
/*$(document).on("click", ".btn_unstorageItems", function() {
	...
	$.ajax({
		...
		dataType: 'json',
		success: function(result) {
			renderUnLoadedModal(result.list);
		},
		error: function(xhr, status, error) {
...
function renderUnLoadedModal(data) {
	const { list = [], total = 0, page = 1, totalPages: pages = 1 } = data;
	...
	const rows = list.map((row, i) => {
		...
		return `
            <tr>
            	<td class = "checkboxVal"><input type="checkbox" class="exceptionOut_chk" 
            		data-unique="${barcode}_${indate}_${factory}_${storage}_${itemcode}_${qty}_${source}"></td>
                <td class="idx">${globalIndex}</td>
                <td class="barcode">${barcode}</td>
                <td class="indate">${indate}</td>
                <td class="car">${car}</td>
                <td class="itemname">${itemname}</td>
                <td class="itemcode">${itemcode}</td>
                <td class="qty">${Number(qty).toLocaleString()}</td>
                <td class="qty">${source}</td>
            </tr>`;
	}).join('');
	...
	const tableHtml = list.length > 0
                    ...
                    <thead>
                        <tr>
                        	<th class = "checkboxVal">
								<input type="checkbox" class="exceptionOut_chkAll">
							</th>
		...
		: `<div class="empty">미적재 데이터가 없습니다.</div>`;

	// 페이징 네비게이션 생성
	const paginationHtml = totalPages > 1 ? createPaginationHtml() : '';
	...
	const modalHtml = `
        <div class="m2_5_2_mainDiv">
            <div class="modal-header" style="margin-bottom:0px !important;">
                <h3 class="modal-title">${i18n.t('normal.unloaded.list')} (Count: ${totalCount})</h3>
				<input type="button" value="Exception Out" class="btn btn-success btnExceptionOut"/>
...
//이벤트 바인딩
function bindUnloadedEvents() {
	// 전체 선택 체크박스
	$(document).off('change', '.exceptionOut_chkAll').on('change', '.exceptionOut_chkAll', function() {
		let isChecked = $(this).is(':checked');
		$('.exceptionOut_chk').prop('checked', isChecked);
	});

	// 개별 체크박스
	$(document).off('change', '.exceptionOut_chk').on('change', '.exceptionOut_chk', function() {
		let totalCheckboxes = $('.exceptionOut_chk').length;
		let checkedCheckboxes = $('.exceptionOut_chk:checked').length;
		$('.exceptionOut_chkAll').prop('checked', totalCheckboxes === checkedCheckboxes);
	});
}
...
function createPaginationHtml() {
	...
	// 이전 버튼
	if (currentPage > 1) {
		paginationHtml += `<button class="pagination-btn" onclick="loadUnloadedPage(1)">처음</button>`;
		paginationHtml += `<button class="pagination-btn" onclick="loadUnloadedPage(${currentPage - 1})">이전</button>`;
	}

	// 페이지 번호 버튼들 (현재 페이지 주변 5개씩 표시)
	const startPage = Math.max(1, currentPage - 2);
	const endPage = Math.min(totalPages, currentPage + 2);

	for (let i = startPage; i <= endPage; i++) {
		const activeClass = i === currentPage ? 'active' : '';
		paginationHtml += `<button class="pagination-btn ${activeClass}" onclick="loadUnloadedPage(${i})">${i}</button>`;
	}

	// 다음 버튼
	if (currentPage < totalPages) {
		paginationHtml += `<button class="pagination-btn" onclick="loadUnloadedPage(${currentPage + 1})">다음</button>`;
		paginationHtml += `<button class="pagination-btn" onclick="loadUnloadedPage(${totalPages})">마지막</button>`;
	}

	paginationHtml += '</div>';

	// 페이지 정보 표시
	paginationHtml += `<div class="pagination-info">페이지 ${currentPage} / ${totalPages} (총 ${totalCount}개)</div>`;

	return paginationHtml;
}

// 전역 함수로 선언
window.loadUnloadedPage = function(page) {
...
/*function changeLocationImg(storage) {
	
	var src = "";
	if(storage == 'Material'){
		src = '../resources/images/locationmap.png';
	}else if(storage = 'Fabric') {
	}else if(storage = 'Side seat') {
		src = '../resources/images/sideseatmap.png';
	}else if(storage = 'Outside') {
		src = '../resources/images/locationmap.png';
	}else if(storage = 'H/REST') {
		src = '../resources/images/locationmap.png';
	}
}*/
