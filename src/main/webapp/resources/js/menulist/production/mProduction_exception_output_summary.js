 ...
 * 
 * 1. 메뉴명 mProduction_exception_output_summary 을 전부 사용하는 메뉴명으로 Replace
...
let totalProductionExceptionOutputSummaryTotalPages = 0; // 서버에서 받은 총 개수 저장
...
$(document).ready(function() {
	...
	window.call_mProduction_exception_output_summary = function(menuId) {
		showLoading("data");
		...
		performProductionExceptionOutputSummaryDBSearch({ fromDate, toDate, factory });
	}
});
...
function performProductionExceptionOutputSummaryDBSearch(searchCriteria) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			totalProductionExceptionOutputSummaryTotalPages = data.totalPages;
			currentProductionExceptionOutputSummaryPage = data.currentPage;

			// 첫 번째 검색이라면 뷰를 렌더링
			if (!$('#view_mProduction_exception_output_summary').length) {
			...
			$(".mProduction_exception_output_summary").siblings('.tabCommon').css('font-size', '9.2pt');
...
function renderProductionExceptionOutputSummaryView() {
	let content_output = `
		<div class="divBlockControl" id="view_mProduction_exception_output_summary">
				...
				<div class="table-container">
					...
					<div class="table-info">
						<span>${i18n.t('table.info.total')} <strong id="productionExceptionOutputSummaryTotalCount">${totalProductionExceptionOutputSummaryCount}</strong> ${i18n.t('table.info.records')} | 
							${i18n.t('table.page')} <strong id="productionExceptionOutputSummaryCurrentPageInfo">${currentProductionExceptionOutputSummaryPage}</strong>/<strong id="productionExceptionOutputSummaryTotalPageInfo">${totalProductionExceptionOutputSummaryTotalPages}</strong> |  
							<span class="tqtyTitle">${i18n.t('table.info.qty')} : </span><span class="productionExceptionOutputSummaryTotalQty" style="color:#007bff"></span>
						</span>
						<div class="action-buttons-right mProduction_exception_output_summary">
					...
					<table class="data-table mProduction_exception_output_summary">
						<thead>
							<tr>
...
function updateProductionExceptionOutputSummaryTotalCount() {
	$('#productionExceptionOutputSummaryTotalCount').text(totalProductionExceptionOutputSummaryCount);
}
...
function renderProductionExceptionOutputSummaryTableData() {
	...
	$("#productionExceptionOutputSummaryCurrentPageInfo").text(currentProductionExceptionOutputSummaryPage);
	$("#productionExceptionOutputSummaryTotalPageInfo").text(totalProductionExceptionOutputSummaryTotalPages);
...
//	});
	...
	// 엔터키 검색
	$('#view_mProduction_exception_output_summary input[type="text"], #view_mProduction_exception_output_summary input[type="date"]').off('keypress').on('keypress', function(e) {
		if (e.which === 13) {
			performProductionExceptionOutputSummarySearch();
		}
	});
...
function updateTotalQty() {
	...
	$.ajax({
		...
		contentType: "application/json",
		success: function(data) {
			$(".productionExceptionOutputSummaryTotalQty").text(Number(data).toLocaleString());
		},
		error: function(xhr, status, error) {
			console.error("요청 실패");
...
//	console.log(iidList)
//	$.ajax({
//		url: "/productionExceptionOutput_confirm_summary",
//		type: "POST",
//		data: JSON.stringify(iidList),
//		contentType: "application/json",
//		success: function(data) {
//			let searchVal = getCurrentSearchCriteria();
//			performProductionExceptionOutputSummaryDBSearch(searchVal);
//			
//			// 전체 선택 해제
...
//	console.log(iidList)
//	$.ajax({
//		url: "/productionExceptionOutput_confirm_summary_cancel",
//		type: "POST",
//		data: JSON.stringify(iidList),
//		contentType: "application/json",
//		success: function(data) {
