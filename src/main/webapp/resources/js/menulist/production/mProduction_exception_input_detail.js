 ...
 * 
 * 1. 메뉴명 mProduction_exception_input_detail 을 전부 사용하는 메뉴명으로 Replace
...
let totalProductionExceptionInputDetailTotalPages = 0; // 서버에서 받은 총 개수 저장
...
$(document).ready(function() {
	...
	window.call_mProduction_exception_input_detail = function(menuId) {
		showLoading("data");
		...
		performProductionExceptionInputDetailDBSearch({ fromDate, toDate, factory });
	}
});
...
function performProductionExceptionInputDetailDBSearch(searchCriteria) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			totalProductionExceptionInputDetailTotalPages = data.totalPages;
			currentProductionExceptionInputDetailPage = data.currentPage;

			// 첫 번째 검색이라면 뷰를 렌더링
			if (!$('#view_mProduction_exception_input_detail').length) {
...
function renderProductionExceptionInputDetailView() {
	let content_output = `
		<div class="divBlockControl" id="view_mProduction_exception_input_detail">
			<div class="content-body">
				<!-- 검색 영역 -->
				<div class="search-area">
					<div class="search-row">
						<div class="search-label" style="width:24%">
							<div class="exception_ID_searchVal_fromToDate">${i18n.t('search.from')}<!-- FromTo --></div>
							<div class="exception_ID_searchVal_fromToDateArea">
								<input type="date" id="exception_ID_searchVal_fromDate"/>
								<span class="middleWave">~</span>
								<input type="date" id="exception_ID_searchVal_toDate"/>
							</div>
						</div>
						<div class="search-label">
							<div class="exception_ID_searchVal_factory">${i18n.t('search.factory')}<!-- FACTORY --></div>
							<select id="exception_ID_searchVal_factory" class="factory-select">
						...
						<div class="search-label">
							<div class="exception_ID_searchVal_storage">${i18n.t('search.storage')}<!-- STORAGE --></div>
							<select id="exception_ID_searchVal_storage" >
								<!-- 동적으로 추가 -->
							</select>
						</div>
						<div class="search-label">
							<div class="exception_ID_searchVal_car">${i18n.t('search.car')}<!-- CAR --></div>
							<input type="text" id="exception_ID_searchVal_car" />
						</div>
						<div class="search-label">
							<div class="exception_ID_searchVal_itemcode">${i18n.t('search.itemCode')}<!-- ITEMCODE --></div>
							<input type="text" id="exception_ID_searchVal_itemcode" />
						</div>
						<div class="search-label">
							<div class="exception_ID_searchVal_itemname">${i18n.t('search.itemName')}<!-- ITEMNAME --></div>
							<input type="text" id="exception_ID_searchVal_itemname" />
						</div>
						<div class="search-label">
							<div class="exception_ID_searchVal_source">${i18n.t('search.type')}<!-- TYPE --></div>
							<select id="exception_ID_searchVal_source" >
				...
				<div class="table-container">
					...
					<div class="table-info">
						<span>${i18n.t('table.info.total')} <strong id="productionExceptionInputDetailTotalCount">${totalProductionExceptionInputDetailCount}</strong> ${i18n.t('table.info.records')} | 
							${i18n.t('table.page')} <strong id="productionExceptionInputDetailCurrentPageInfo">${currentProductionExceptionInputDetailPage}</strong>/<strong id="productionExceptionInputDetailTotalPageInfo">${totalProductionExceptionInputDetailTotalPages}</strong> |  
							<span class="tqtyTitle">${i18n.t('table.info.qty')} : </span><span class="productionExceptionInputDetailTotalQty" style="color:#007bff"></span>
						</span>
						<div class="action-buttons-right mProduction_exception_input_detail">
							<div id="defaultActions" class="action-group">
								<input type="button" value="${i18n.t('btn.delete')}" class="btn btn-danger btnProductionExceptionInputDelete"/>
						...
						<div class="btnInterfaceCommon btnProductionExceptionInputItemsArea" style="margin-left:24px;">
							<input type="button" value="${i18n.t('btn.Intf')}" class="btn btn-success btnIntfProductionExceptionInput"/>
							<input type="button" value="${i18n.t('btn.Intf.delete')}" class="btn btn-warning btnIntfProductionExceptionInputDelete"/>
						</div>
					</div>
					<table class="data-table mProduction_exception_input_detail">
						<thead>
							<tr>
								<th class = "checkboxVal">
									<input type="checkbox" class="productionExceptionInput_chkAll">
								</th>
	...
	`;
	/*<div class="search-label">
							<div class="exception_ID_searchVal_username">${i18n.t('search.username')}<!-- USERNAME --></div>
							<input type="text" id="exception_ID_searchVal_username" />
						</div>*/
	...
	(function() {
		const { fromDate, toDate } = getDefaultDateRange();
		$("#exception_ID_searchVal_fromDate").val(fromDate);
		$("#exception_ID_searchVal_toDate").val(toDate);
	})();
...
function renderFactoryStorage() {
    const factory = $('#exception_ID_searchVal_factory');
    const storage = $('#exception_ID_searchVal_storage');
...
function renderProductionExceptionInputDetailTableData() {
	...
	$("#productionExceptionInputDetailCurrentPageInfo").text(currentProductionExceptionInputDetailPage);
	$("#productionExceptionInputDetailTotalPageInfo").text(totalProductionExceptionInputDetailTotalPages);
	...
	for (let i = 0; i < globalProductionExceptionInputDetailData.length; i++) {
		...
		//console.log(`행 ${i}:`, globalProductionExceptionInputDetailData[i]); // 각 행 데이터 확인
		...
		tableBody += `
            <tr>
			    <td class = "checkboxVal"><input type="checkbox" class="productionExceptionInput_chk ${statusClass}" 
	...
	$(".productionExceptionInput_chkAll").prop("checked", false);
	
	// 📌 데이터 렌더링 후 Memo 컬럼 자동 폭 조정
    adjustProductionExceptionInputDetailColWidth('memoVal', '.memoText');
}
...
function bindProductionExceptionInputDetailEvents() {
	// 전체 선택 체크박스
	$(document).off('change', '.productionExceptionInput_chkAll').on('change', '.productionExceptionInput_chkAll', function() {
		let isChecked = $(this).is(':checked');
		$('.productionExceptionInput_chk').prop('checked', isChecked);
	});

	// 개별 체크박스
	$(document).off('change', '.productionExceptionInput_chk').on('change', '.productionExceptionInput_chk', function() {
		let totalCheckboxes = $('.productionExceptionInput_chk').length;
		let checkedCheckboxes = $('.productionExceptionInput_chk:checked').length;
		$('.productionExceptionInput_chkAll').prop('checked', totalCheckboxes === checkedCheckboxes);
	});
	...
	// 엔터키 검색
	$('#view_mProduction_exception_input_detail input[type="text"], #view_mProduction_exception_input_detail input[type="date"]').off('keypress').on('keypress', function(e) {
		if (e.which === 13) {
			performProductionExceptionInputDetailSearch();
		}
	});
...
function getCurrentSearchCriteria() {
	return {
		fromDate: $("#exception_ID_searchVal_fromDate").val(),
		toDate: $("#exception_ID_searchVal_toDate").val(),
		factory: $("#exception_ID_searchVal_factory").val(),
		storage: $("#exception_ID_searchVal_storage").val(),
		car: $("#exception_ID_searchVal_car").val().trim().toUpperCase(),
		itemcode: $("#exception_ID_searchVal_itemcode").val().trim().toUpperCase(),
		itemname: $("#exception_ID_searchVal_itemname").val().trim().toUpperCase(),
		source: $('#exception_ID_searchVal_source').val()
	};
}
// =
// 검색 수행 함수 - DB 조회
...
function resetProductionExceptionInputDetailSearch() {
	...
	$("#exception_ID_searchVal_fromDate").val(fromDate);
	$("#exception_ID_searchVal_toDate").val(toDate);
	$("#exception_ID_searchVal_factory").val(factory);
	$("#exception_ID_searchVal_storage").val('Material');
	$("#exception_ID_searchVal_car").val('');
	$("#exception_ID_searchVal_itemcode").val('');
	$("#exception_ID_searchVal_itemname").val('');
	$('#exception_ID_searchVal_source').val('all')
	// 초기화 후 전체 데이터 다시 조회
	currentProductionExceptionInputDetailPage = 1;
	performProductionExceptionInputDetailDBSearch({ fromDate, toDate, factory });
...
//📌 공통: 특정 컬럼(헤더+셀) 폭을 "가장 긴 텍스트" 기준으로 맞추는 함수
function adjustProductionExceptionInputDetailColWidth(colClass, innerTextClass) {
    var $header = $('table.data-table.mProduction_exception_input_detail th.' + colClass);
    var $cells  = $('#productionExceptionInputDetailTableBody td.' + colClass);

    if ($cells.length === 0) return;

    // 이전 설정 초기화
    $header.css({ width: '', 'min-width': '' });
    $cells.css({ width: '', 'min-width': '' });

    var maxWidth = 0;

    // 실제 텍스트 span 기준 폭 측정
    $('#productionExceptionInputDetailTableBody td.' + colClass + ' ' + innerTextClass).each(function () {
        var w = this.scrollWidth; // 내용 기준 실제 필요 폭
        if (w > maxWidth) {
            maxWidth = w;
        }
    });
...
function updateTotalQty() {
	...
	$.ajax({
		...
		contentType: "application/json",
		success: function(data) {
			$(".productionExceptionInputDetailTotalQty").text(Number(data).toLocaleString());
		},
		error: function(xhr, status, error) {
			console.error("요청 실패");
...
window.downloadAllProductionExceptionInputDetailData = function() {
	...
	$.ajax({
		url: "/read_exceptionInputDetail_all",
		type: "POST",
		data: JSON.stringify({
			searchParams: searchCriteria
		}),
...
// 삭제
$(document).on("click", ".btnProductionExceptionInputDelete", function() {	
	if ($(".productionExceptionInput_chk.status-completed:checked").length > 0) {
	...
	const iidList = [];
	$(".productionExceptionInput_chk:checked").each(function() {
	...
	console.log(iidList)
	$.ajax({
		url: "/deleteProductionExceptionInput",
		type: "POST",
		data: JSON.stringify({
			iidList : iidList,
			loginid : loginid
		...
		success: function(data) {
			...
			performProductionExceptionInputDetailDBSearch(searchVal);	
...
//인터페이스
$(document).on("click", ".btnIntfProductionExceptionInput", function() {
	...
	console.log(iidList)
	$.ajax({
		url: "/productionExceptionInput_confirm_summary",
		type: "POST",
...
$(document).on("click", ".btnIntfProductionExceptionInputDelete", function() {

	if ($(".productionExceptionInput_chk.status-waiting:checked").length > 0) {
