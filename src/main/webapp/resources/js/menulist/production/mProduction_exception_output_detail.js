 ...
 * 
 * 1. 메뉴명 mProduction_exception_output_detail 을 전부 사용하는 메뉴명으로 Replace
...
let totalProductionExceptionOutputDetailTotalPages = 0; // 서버에서 받은 총 개수 저장
...
$(document).ready(function() {
	...
	window.call_mProduction_exception_output_detail = function(menuId) {
		showLoading("data");
		...
		performProductionExceptionOutputDetailDBSearch({ fromDate, toDate, factory });
	}
});
...
function performProductionExceptionOutputDetailDBSearch(searchCriteria) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			totalProductionExceptionOutputDetailTotalPages = data.totalPages;
			currentProductionExceptionOutputDetailPage = data.currentPage;

			// 첫 번째 검색이라면 뷰를 렌더링
			if (!$('#view_mProduction_exception_output_detail').length) {
...
function renderProductionExceptionOutputDetailView() {
	let content_output = `
		<div class="divBlockControl" id="view_mProduction_exception_output_detail">
			<div class="content-body">
				<!-- 검색 영역 -->
				<div class="search-area">
					<div class="search-row">
						<div class="search-label" style="width:32%">
							<div class="exception_OD_searchVal_fromToDate">${i18n.t('search.from')}<!-- FromTo --></div>
							<div class="exception_OD_searchVal_fromToDateArea">
								<input type="date" id="exception_OD_searchVal_fromDate"/>
								<span class="middleWave">~</span>
								<input type="date" id="exception_OD_searchVal_toDate"/>
							</div>
						</div>
						<div class="search-label">
							<div class="exception_OD_searchVal_factory">${i18n.t('search.factory')}<!-- FACTORY --></div>
							<select id="exception_OD_searchVal_factory" class="factory-select">
						...
						<div class="search-label">
							<div class="exception_OD_searchVal_storage">${i18n.t('search.storage')}<!-- STORAGE --></div>
							<select id="exception_OD_searchVal_storage" >
								<!-- 동적으로 추가 -->
							</select>
						</div>
						<div class="search-label">
							<div class="exception_OD_searchVal_car">${i18n.t('search.car')}<!-- CAR --></div>
							<input type="text" id="exception_OD_searchVal_car" />
						</div>
						<div class="search-label">
							<div class="exception_OD_searchVal_itemcode">${i18n.t('search.itemCode')}<!-- ITEMCODE --></div>
							<input type="text" id="exception_OD_searchVal_itemcode" />
						</div>
						<div class="search-label">
							<div class="exception_OD_searchVal_itemname">${i18n.t('search.itemName')}<!-- ITEMNAME --></div>
							<input type="text" id="exception_OD_searchVal_itemname" />
						</div>
						<div class="search-label">
							<div class="exception_OD_searchVal_source">${i18n.t('search.type')}<!-- TYPE --></div>
							<select id="exception_OD_searchVal_source" >
				...
				<div class="table-container">
					...
					<div class="table-info">
						<span>${i18n.t('table.info.total')} <strong id="productionExceptionOutputDetailTotalCount">${totalProductionExceptionOutputDetailCount}</strong> ${i18n.t('table.info.records')} | 
							${i18n.t('table.page')} <strong id="productionExceptionOutputDetailCurrentPageInfo">${currentProductionExceptionOutputDetailPage}</strong>/<strong id="productionExceptionOutputDetailTotalPageInfo">${totalProductionExceptionOutputDetailTotalPages}</strong> |  
							<span class="tqtyTitle">${i18n.t('table.info.qty')} : </span><span class="productionExceptionOutputDetailTotalQty" style="color:#007bff"></span>
						</span>
						<div class="action-buttons-right mProduction_exception_output_detail">
							<div id="defaultActions" class="action-group">
								<input type="button" value="${i18n.t('btn.delete')}" class="btn btn-danger btnProductionExceptionOutputDelete"/>
						...
						<div class="btnInterfaceCommon btnProductionExceptionOutputItemsArea" style="margin-left:24px;">
							<input type="button" value="${i18n.t('btn.Intf')}" class="btn btn-success btnIntfProductionExceptionOutput"/>
							<input type="button" value="${i18n.t('btn.Intf.delete')}" class="btn btn-warning btnIntfProductionExceptionOutputDelete"/>
						</div>
					</div>
					<table class="data-table mProduction_exception_output_detail">
						<thead>
							<tr>
								<th class = "checkboxVal">
									<input type="checkbox" class="productionExceptionOutput_chkAll">
								</th>
								...
								<th class = "barcodeVal">${i18n.t('search.type')}<!-- type --></th>
	...
	`;
	/*<div class="search-label">
							<div class="exception_OD_searchVal_username">${i18n.t('search.username')}<!-- USERNAME --></div>
							<input type="text" id="exception_OD_searchVal_username" />
						</div>*/
	...
	(function() {
		const { fromDate, toDate } = getDefaultDateRange();
		$("#exception_OD_searchVal_fromDate").val(fromDate);
		$("#exception_OD_searchVal_toDate").val(toDate);
	})();
...
function renderFactoryStorage() {
    const factory = $('#exception_OD_searchVal_factory');
    const storage = $('#exception_OD_searchVal_storage');
...
function renderProductionExceptionOutputDetailTableData() {
	...
	$("#productionExceptionOutputDetailCurrentPageInfo").text(currentProductionExceptionOutputDetailPage);
	$("#productionExceptionOutputDetailTotalPageInfo").text(totalProductionExceptionOutputDetailTotalPages);
	...
	for (let i = 0; i < globalProductionExceptionOutputDetailData.length; i++) {
		...
		//console.log(`행 ${i}:`, globalProductionExceptionOutputDetailData[i]); // 각 행 데이터 확인
		...
		tableBody += `
            <tr>
            	<td class = "checkboxVal"><input type="checkbox" class="productionExceptionOutput_chk ${statusClass}" 
				...
				<td class = "barcodeVal">${source2}</td>
				<td class = "memoVal">
	...
	$(".productionExceptionOutput_chkAll").prop("checked", false);
	
	// 📌 데이터 렌더링 후 Memo 컬럼 자동 폭 조정
	adjustProductionExceptionOutputDetailMemoWidth();
}
...
function renderProductionExceptionOutputDetailPagination() {
	...
	$("#productionExceptionOutputDetailCurrentPageInfo").html(currentProductionExceptionOutputDetailPage);
...
function bindProductionExceptionOutputDetailEvents() {
	// 전체 선택 체크박스
	$(document).off('change', '.productionExceptionOutput_chkAll').on('change', '.productionExceptionOutput_chkAll', function() {
		let isChecked = $(this).is(':checked');
		$('.productionExceptionOutput_chk').prop('checked', isChecked);
	});

	// 개별 체크박스
	$(document).off('change', '.productionExceptionOutput_chk').on('change', '.productionExceptionOutput_chk', function() {
		let totalCheckboxes = $('.productionExceptionOutput_chk').length;
		let checkedCheckboxes = $('.productionExceptionOutput_chk:checked').length;
		$('.productionExceptionOutput_chkAll').prop('checked', totalCheckboxes === checkedCheckboxes);
	});
	...
	// 엔터키 검색
	$('#view_mProduction_exception_output_detail input[type="text"], #view_mProduction_exception_output_detail input[type="date"]').off('keypress').on('keypress', function(e) {
		if (e.which === 13) {
			performProductionExceptionOutputDetailSearch();
		}
	});
...
function getCurrentSearchCriteria() {
	return {
		fromDate: $("#exception_OD_searchVal_fromDate").val(),
		toDate: $("#exception_OD_searchVal_toDate").val(),
		factory: $("#exception_OD_searchVal_factory").val(),
		storage: $("#exception_OD_searchVal_storage").val(),
		car: $("#exception_OD_searchVal_car").val().trim().toUpperCase(),
		itemcode: $("#exception_OD_searchVal_itemcode").val().trim().toUpperCase(),
		itemname: $("#exception_OD_searchVal_itemname").val().trim().toUpperCase(),
		source: $("#exception_OD_searchVal_source").val()
	};
}
// =
// 검색 수행 함수 - DB 조회
...
function resetProductionExceptionOutputDetailSearch() {
	...
	$("#exception_OD_searchVal_fromDate").val(fromDate);
	$("#exception_OD_searchVal_toDate").val(toDate);
	$("#exception_OD_searchVal_factory").val(factory);
	$("#exception_OD_searchVal_storage").val('Material');
	$("#exception_OD_searchVal_car").val('');
	$("#exception_OD_searchVal_itemcode").val('');
	$("#exception_OD_searchVal_itemname").val('');
	$("#exception_OD_searchVal_source").val('all');
	// 초기화 후 전체 데이터 다시 조회
	currentProductionExceptionOutputDetailPage = 1;
	performProductionExceptionOutputDetailDBSearch({ fromDate, toDate, factory });
...
function adjustProductionExceptionOutputDetailMemoWidth() {
  var $memoHeader = $('table.data-table.mProduction_exception_output_detail th.memoVal');
  var $memoCells  = $('#productionExceptionOutputDetailTableBody td.memoVal');
  ...
  $('#productionExceptionOutputDetailTableBody td.memoVal .memoText').each(function () {
...
function updateTotalQty() {
	...
	$.ajax({
		...
		contentType: "application/json",
		success: function(data) {
			$(".productionExceptionOutputDetailTotalQty").text(Number(data).toLocaleString());
		},
		error: function(xhr, status, error) {
			console.error("요청 실패");
...
window.downloadAllProductionExceptionOutputDetailData = function() {
	...
	$.ajax({
		url: "/read_exceptionOutputDetail_all",
		type: "POST",
		data: JSON.stringify({
			searchParams: searchCriteria
		}),
		...
		success: function(data) {
			console.log(data)
			data.forEach(row => {
			   // row.INVOICENO가 'BARCODECOUNT'면 STOCKCOUNT, 아니면 EXCEPTION
		       row.TYPE = row.INVOICENO === 'BARCODECOUNT' ? 'STOCKCOUNT' : 'EXCEPTION';
		   });
...
//삭제
$(document).on("click", ".btnProductionExceptionOutputDelete", function() {	
	if ($(".productionExceptionOutput_chk.status-completed:checked").length > 0) {
	...
	const iidList = [];
	$(".productionExceptionOutput_chk:checked").each(function() {
	...
	$.ajax({
		...
		success: function(data) {
			...
			performProductionExceptionOutputDetailDBSearch(searchVal);	
...
//인터페이스 등록
$(document).on("click", ".btnIntfProductionExceptionOutput", function() {	
	if ($(".productionExceptionOutput_chk.status-completed:checked").length > 0) {
...
//인터페이스 등록 취소
$(document).on("click", ".btnIntfProductionExceptionOutputDelete", function() {
	if ($(".productionExceptionOutput_chk.status-waiting:checked").length > 0) {
