 ...
 * 
 * 1. 메뉴명 mProduction_exception_input_summary 을 전부 사용하는 메뉴명으로 Replace
...
let totalProductionExceptionInputSummaryTotalPages = 0; // 서버에서 받은 총 개수 저장
...
$(document).ready(function() {	
	...
	window.call_mProduction_exception_input_summary = function(menuId) {
		showLoading("data");
		...
		performProductionExceptionInputSummaryDBSearch({ fromDate, toDate, factory });
	}
});
...
function performProductionExceptionInputSummaryDBSearch(searchCriteria) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			totalProductionExceptionInputSummaryTotalPages = data.totalPages;
			currentProductionExceptionInputSummaryPage = data.currentPage;

			// 첫 번째 검색이라면 뷰를 렌더링
			if (!$('#view_mProduction_exception_input_summary').length) {
...
function renderProductionExceptionInputSummaryView() {
	let content_output = `
		<div class="divBlockControl" id="view_mProduction_exception_input_summary">
				...
				<div class="table-container">
					...
					<div class="table-info">
						<span>${i18n.t('table.info.total')} <strong id="productionExceptionInputSummaryTotalCount">${totalProductionExceptionInputSummaryCount}</strong> ${i18n.t('table.info.records')} | 
							${i18n.t('table.page')} <strong id="productionExceptionInputSummaryCurrentPageInfo">${currentProductionExceptionInputSummaryPage}</strong>/<strong id="productionExceptionInputSummaryTotalPageInfo">${totalProductionExceptionInputSummaryTotalPages}</strong> |  
							<span class="tqtyTitle">${i18n.t('table.info.qty')} : </span><span class="productionExceptionInputSummaryTotalQty" style="color:#007bff"></span>
						</span>
						<div class="action-buttons-right mProduction_exception_input_summary">
					...
					<table class="data-table mProduction_exception_input_summary">
						<thead>
							<tr>
...
function updateProductionExceptionInputSummaryTotalCount() {
	$('#productionExceptionInputSummaryTotalCount').text(totalProductionExceptionInputSummaryCount);
}
...
function renderProductionExceptionInputSummaryTableData() {
	...
	$("#productionExceptionInputSummaryCurrentPageInfo").text(currentProductionExceptionInputSummaryPage);
	$("#productionExceptionInputSummaryTotalPageInfo").text(totalProductionExceptionInputSummaryTotalPages);
...
//	});
	...
	// 엔터키 검색
	$('#view_mProduction_exception_input_summary input[type="text"], #view_mProduction_exception_input_summary input[type="date"]').off('keypress').on('keypress', function(e) {
		if (e.which === 13) {
			performProductionExceptionInputSummarySearch();
		}
	});
...
function updateTotalQty() {
	...
	$.ajax({
		...
		contentType: "application/json",
		success: function(data) {
			$(".productionExceptionInputSummaryTotalQty").text(Number(data).toLocaleString());
		},
		error: function(xhr, status, error) {
			console.error("요청 실패");
...
//			performProductionExceptionInputSummaryDBSearch(searchVal);	
//			
//			// 전체 선택 해제
...
//	console.log(iidList)
//	$.ajax({
//		url: "/productionExceptionInput_confirm_summary_cancel",
//		type: "POST",
//		data: JSON.stringify(iidList),
//		contentType: "application/json",
//		success: function(data) {
