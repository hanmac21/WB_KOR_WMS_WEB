...
/*(function() {
	...
	$(document).ready(function() {
		...
		window.filteredfactoryCompleteData = [];
		window.factoryCompleteColumns = [
			...
			{ key: 'COMPLETE', header: 'Complete' }              // 완료여부
		];
		...
		function performfactoryCompleteDBSearch(searchCriteria) {
			...
			$.ajax({
				...
				success: function(response) {
					...
					filteredData_purchaseFactoryComplete = [...allServerData]; // 초기에는 필터링 없음
					...
					// 첫 번째 검색이라면 뷰를 렌더링
					if (!$('#view_mPurchase_factory_complete').length) {
					...
					<div class="table-container">
						...
						<div class="table-info">
							<span>${i18n.t('table.info.total')} <strong id="factoryCompleteTotalCount">${totalfactoryCompleteCount}</strong> ${i18n.t('table.info.records')} | 
								${i18n.t('table.page')} <strong id="factoryCompleteCurrentPageInfo">${currentfactoryCompletePage}</strong>/<strong id="factoryCompleteTotalPageInfo">${totalfactoryCompletePages}</strong> |  
								<!-- <span class="tqtyTitle">${i18n.t('table.info.qty')} : </span><span class="factoryCompleteTotalQty" style="color:#007bff"></span> --> 
							</span>
						...
						<table class="data-table mPurchase_factory_complete" id="factoryCompleteTable">
							<thead>
								<tr>
									...
									<th class="dateVal" data-sort="COMPLETE"><!-- LOT -->Complete</th>
		...
		`;
			...
			(function() {
				...
				$("#factoryComplete_searchVal_storage").val('all');
		...
		function renderFactoryStorage() {
			...
			function updateStorageOptions(factoryValue) {
				...
				const options = {
					'SALTILLO': ['Material', 'Fabric', 'Side seat', 'Outside', 'Material+Sideseat+Outside', 'P1 W/HOUSE', 'all'],
					'PUEBLA': ['Material', 'PRODUCT', 'all'],
					'all': ['Material', 'Fabric', 'Side seat', 'Outside', 'PRODUCT', 'Material+Sideseat+Outside', 'P1 W/HOUSE', 'all']
				};
				...
				if (menuType === "purchase") {
					...
					$("#factoryComplete_searchVal_storage").val('Material');
				} else if (menuType === "fabric") {
					saveStorageForInit = "Fabric";
					$("#factoryComplete_searchVal_storage").val('Fabric');
				} else if (menuType === "sales") {
					saveStorageForInit = "P1 W/HOUSE";
					$("#factoryComplete_searchVal_storage").val('P1 W/HOUSE');
				}
			}
		...
		function renderfactoryCompleteTableData() {
            ...
            <tr>
                ...
                <td class="wccodeVal">${globalfactoryCompleteData[i].FROMSTORAGE || globalfactoryCompleteData[i].fromstorage || ''}</td>
                <td class="dateVal">${globalfactoryCompleteData[i].COMPLETE || globalfactoryCompleteData[i].complete || ''}</td>
		...
		function bindfactoryCompleteEvents() {
			...
			$('#factoryComplete_itemsPerPage').off('change').on('change', function() {
				...
				changeFactoryCompleteItemsPerPage(newItemsPerPage);
			});
		...
		function getCurrentSearchCriteria() {
			return {
				...
				car: ($("#factoryComplete_searchVal_car").val() || '').trim().toUpperCase(),
		...
		function resetfactoryCompleteSearch() {
			...
			$("#factoryComplete_searchVal_fromDate").val(fromDate);
			$("#factoryComplete_searchVal_toDate").val(toDate);
			$("#factoryComplete_searchVal_car").val('');
	...
	window.downloadAllfactoryCompleteData = function() {
		...
		$.ajax({
			...
			success: function(response) {
				...
				ExcelExporter.downloadExcel(filteredData_purchaseFactoryComplete, window.factoryCompleteColumns, {
	...
	$(document).on("click", ".btnFactoryCompleteSearch", function() {
		alert("ss1")
	})
})();*/