...
// * 
// * 1. 메뉴명 m2_exception_input_detail 을 전부 사용하는 메뉴명으로 Replace
// * 2. 앞글자 소문자 전부 메뉴영어명으로 Replace : exceptionInputDetail -> newMenuName
// * 3. 앞글대 대문자 전부 메뉴영어명으로 Replace : ExceptionInputDetail -> NewMenuName
// * 4. 표시된 오류 및 = 부분 수정
...
// * -------------------------------------------------------------- */
//
//let globalExceptionInputDetailData = []; // 현재 조회된 데이터 저장
//let currentExceptionInputDetailPage = 1; // 현재 페이지
//let exceptionInputDetailItemsPerPage = 1000; // 페이지당 항목 수
//let totalExceptionInputDetailCount = 0; // 서버에서 받은 총 개수 저장
//let totalExceptionInputDetailTotalPages = 0; // 서버에서 받은 총 개수 저장
//window.filteredExceptionInputDetailData = []; // 현재 표시되는 데이터 (엑셀 다운로드용)
...
//	window.call_m2_exception_input_detail = function(menuId) {
//		showLoading("data");
//
...
//		performExceptionInputDetailDBSearch({ fromDate, toDate, factory });
//	}
//});
//
//// DB에서 데이터 조회하는 함수
...
//			page: currentExceptionInputDetailPage,
...
//			console.log(data);
//
//			globalExceptionInputDetailData = data.records || data || []; // 서버 응답 구조에 맞게 조정
//			totalExceptionInputDetailCount = data.totalCount || 0;
//			window.filteredExceptionInputDetailData = globalExceptionInputDetailData;
//
//			totalExceptionInputDetailTotalPages = data.totalPages;
//			currentExceptionInputDetailPage = data.currentPage;
//
//			// 첫 번째 검색이라면 뷰를 렌더링
//			if (!$('#view_m2_exception_input_detail').length) {
...
//	if (loginid == "wms") {
//		btnHtml = `
//            <input type="button" value="관리자용 삭제" class="btn btn-danger btnAdminExceptionInputDelete"/>
//        `;
//	}
//	let content_output = `
//		<div class="divBlockControl" id="view_m2_exception_input_detail">
...
//							${i18n.t('table.page')} <strong id="exceptionInputDetailCurrentPageInfo">${currentExceptionInputDetailPage}</strong>/<strong id="exceptionInputDetailTotalPageInfo">${totalExceptionInputDetailTotalPages}</strong> |  
//							<span class="tqtyTitle">${i18n.t('table.info.qty')} : </span><span class="exceptionInputDetailTotalQty" style="color:#007bff"></span>
//						</span>
//						<div class="action-buttons-right m2_exception_input_detail">
//							<div id="defaultActions" class="action-group">
//								${btnHtml}
//								<input type="button" value="${i18n.t('btn.delete')}" class="btn btn-danger btnExceptionInputDelete"/>
...
//						</div>
//					</div>
//					<table class="data-table m2_exception_input_detail">
//						<thead>
//							<tr>
...
//	/*<button class="btn btn-success" id="exceptionInputDetailExcelBtn" onclick="ExcelExporter.downloadExcel(filteredExceptionInputDetailData, exceptionInputDetailColumns, {fileName:'ExceptionInputDetail', sheetName:'ExceptionInputDetail'})">Excel</button>*/
...
//	$('#exceptionInputDetailTotalCount').text(totalExceptionInputDetailCount.toLocaleString());
//}
//
//function renderExceptionInputDetailTableData() {
//	let tableBody = "";
//
//	//console.log("globalExceptionInputDetailData:", globalExceptionInputDetailData);
//	//console.log("데이터 개수:", globalExceptionInputDetailData.length);
//
//	$("#exceptionInputDetailCurrentPageInfo").text(currentExceptionInputDetailPage);
//	$("#exceptionInputDetailTotalPageInfo").text(totalExceptionInputDetailTotalPages);
...
//		//console.log(`행 ${i}:`, globalExceptionInputDetailData[i]); // 각 행 데이터 확인
//		
//		let source2 = "";
//		// source2 값 코드 -> 사유 변환
...
//    adjustExceptionInputDetailColWidth('memoVal', '.memoText');
//}
//
//// 페이지네이션 렌더링
//function renderExceptionInputDetailPagination() {
//	let totalPages = Math.ceil(totalExceptionInputDetailCount / exceptionInputDetailItemsPerPage); // 변경
//	let paginationHtml = "";
//
//	// 이전 버튼
...
//	let endPage = Math.min(totalPages, currentExceptionInputDetailPage + 5);
//
//	// 첫 페이지
//	if (startPage > 1) {
...
//			paginationHtml += `<span class="page-dots">...</span>`;
//		}
//		paginationHtml += `<button class="exceptionInputDetail-page-btn" data-page="${totalPages}">${totalPages}</button>`;
//	}
//
//	// 다음 버튼
//	if (currentExceptionInputDetailPage < totalPages) {
//		paginationHtml += `<button class="exceptionInputDetail-page-btn" data-page="${currentExceptionInputDetailPage + 1}">&gt;</button>`;
//	} else {
//		paginationHtml += `<button class="exceptionInputDetail-page-btn disabled">&gt;</button>`;
//	}
...
//
//	// 엔터키 검색
//	$('#view_m2_exception_input_detail input[type="text"], #view_m2_exception_input_detail input[type="date"]').off('keypress').on('keypress', function(e) {
//		if (e.which === 13) {
//			performExceptionInputDetailSearch();
//		}
//	});
...
//	return {
//		total: globalExceptionInputDetailData.length,
//		currentPage: currentExceptionInputDetailPage,
//		itemsPerPage: exceptionInputDetailItemsPerPage,
//		data: globalExceptionInputDetailData
//	};
//}
//
////📌 공통: 특정 컬럼(헤더+셀) 폭을 "가장 긴 텍스트" 기준으로 맞추는 함수
//function adjustExceptionInputDetailColWidth(colClass, innerTextClass) {
//    var $header = $('table.data-table.m2_exception_input_detail th.' + colClass);
//    var $cells  = $('#exceptionInputDetailTableBody td.' + colClass);
//
...
//
//    // 실제 텍스트 span 기준 폭 측정
//    $('#exceptionInputDetailTableBody td.' + colClass + ' ' + innerTextClass).each(function () {
...
//		contentType: "application/json",
//		success: function(data) {
//			$(".exceptionInputDetailTotalQty").text(Number(data).toLocaleString());
//		},
//		error: function(xhr, status, error) {
//			console.error("요청 실패");
...
//		success: function(data) {
//			console.log(data)
//			ExcelExporter.downloadExcel(data, window.exceptionInputDetailColumns, {
...
//
//// 삭제
//$(document).on("click", ".btnExceptionInputDelete", function() {	
//	if ($(".exceptionInput_chk.status-completed:checked").length > 0) {
...
//
//// 관리자용 삭제
//$(document).on("click", ".btnAdminExceptionInputDelete", function() {	
//	if ($(".exceptionInput_chk.status-completed:checked").length > 0) {
//		alert(i18n.t('validation.confirm.items.delete'));
//		// 확정된 항목이 있습니다. 인퍼테이스 삭제 후 항목을 삭제할 수 있습니다.
//		return;
...
//
//	if ($(".exceptionInput_chk.status-waiting:checked").length > 0) {
