...
// * 
// * 1. 메뉴명 m2_exception_output_detail 을 전부 사용하는 메뉴명으로 Replace
// * 2. 앞글자 소문자 전부 메뉴영어명으로 Replace : exceptionOutputDetail -> newMenuName
// * 3. 앞글대 대문자 전부 메뉴영어명으로 Replace : ExceptionOutputDetail -> NewMenuName
// * 4. 표시된 오류 및 = 부분 수정
...
// * -------------------------------------------------------------- */
//let globalExceptionOutputDetailData = []; // 현재 조회된 데이터 저장
//let currentExceptionOutputDetailPage = 1; // 현재 페이지
//let exceptionOutputDetailItemsPerPage = 1000; // 페이지당 항목 수
//let totalExceptionOutputDetailCount = 0; // 서버에서 받은 총 개수 저장
//let totalExceptionOutputDetailTotalPages = 0; // 서버에서 받은 총 개수 저장
//window.filteredExceptionOutputDetailData = []; // 현재 표시되는 데이터 (엑셀 다운로드용)
...
//	window.call_m2_exception_output_detail = function(menuId) {
//		showLoading("data");
//
...
//		performExceptionOutputDetailDBSearch({ fromDate, toDate, factory });
//	}
//});
//
//// DB에서 데이터 조회하는 함수
...
//			page: currentExceptionOutputDetailPage,
...
//			console.log(data);
//
//			globalExceptionOutputDetailData = data.records || data || []; // 서버 응답 구조에 맞게 조정
//			totalExceptionOutputDetailCount = data.totalCount || 0;
//			window.filteredExceptionOutputDetailData = globalExceptionOutputDetailData;
//
//			totalExceptionOutputDetailTotalPages = data.totalPages;
//			currentExceptionOutputDetailPage = data.currentPage;
//
//			// 첫 번째 검색이라면 뷰를 렌더링
//			if (!$('#view_m2_exception_output_detail').length) {
...
//	if (loginid == "wms") {
//		btnHtml = `
//            <input type="button" value="관리자용 삭제" class="btn btn-danger btnAdminExceptionOutputDelete"/>
//        `;
//	}
//	
//	let content_output = `
//		<div class="divBlockControl" id="view_m2_exception_output_detail">
...
//							${i18n.t('table.page')} <strong id="exceptionOutputDetailCurrentPageInfo">${currentExceptionOutputDetailPage}</strong>/<strong id="exceptionOutputDetailTotalPageInfo">${totalExceptionOutputDetailTotalPages}</strong> |  
//							<span class="tqtyTitle">${i18n.t('table.info.qty')} : </span><span class="exceptionOutputDetailTotalQty" style="color:#007bff"></span>
//						</span>
//						<div class="action-buttons-right m2_exception_output_detail">
//							<div id="defaultActions" class="action-group">
//								${btnHtml}
//								<input type="button" value="${i18n.t('btn.delete')}" class="btn btn-danger btnExceptionOutputDelete"/>
...
//						</div>
//					</div>
//					<table class="data-table m2_exception_output_detail">
//						<thead>
//							<tr>
...
//	/*<button class="btn btn-success" id="exceptionOutputDetailExcelBtn" onclick="ExcelExporter.downloadExcel(filteredExceptionOutputDetailData, exceptionOutputDetailColumns, {fileName:'ExceptionOutputDetail', sheetName:'ExceptionOutputDetail'})">Excel</button>*/
...
//	$('#exceptionOutputDetailTotalCount').text(totalExceptionOutputDetailCount.toLocaleString());
//}
//
//function renderExceptionOutputDetailTableData() {
//	let tableBody = "";
//
//	//console.log("globalExceptionOutputDetailData:", globalExceptionOutputDetailData);
//	//console.log("데이터 개수:", globalExceptionOutputDetailData.length);
//
//	$("#exceptionOutputDetailCurrentPageInfo").text(currentExceptionOutputDetailPage);
//	$("#exceptionOutputDetailTotalPageInfo").text(totalExceptionOutputDetailTotalPages);
...
//		//console.log(`행 ${i}:`, globalExceptionOutputDetailData[i]); // 각 행 데이터 확인
//		
//		let source2 = "";
//		// source2 값 코드 -> 사유 변환
...
//	adjustExceptionOutputDetailMemoWidth();
//}
//
//// 페이지네이션 렌더링
//function renderExceptionOutputDetailPagination() {
//	let totalPages = Math.ceil(totalExceptionOutputDetailCount / exceptionOutputDetailItemsPerPage); // 변경
//	let paginationHtml = "";
//
//	// 이전 버튼
...
//	let endPage = Math.min(totalPages, currentExceptionOutputDetailPage + 5);
//
//	// 첫 페이지
//	if (startPage > 1) {
...
//			paginationHtml += `<span class="page-dots">...</span>`;
//		}
//		paginationHtml += `<button class="exceptionOutputDetail-page-btn" data-page="${totalPages}">${totalPages}</button>`;
//	}
//
//	// 다음 버튼
//	if (currentExceptionOutputDetailPage < totalPages) {
//		paginationHtml += `<button class="exceptionOutputDetail-page-btn" data-page="${currentExceptionOutputDetailPage + 1}">&gt;</button>`;
//	} else {
//		paginationHtml += `<button class="exceptionOutputDetail-page-btn disabled">&gt;</button>`;
//	}
//
//	$("#exceptionOutputDetailCurrentPageInfo").html(currentExceptionOutputDetailPage);
...
//
//	// 엔터키 검색
//	$('#view_m2_exception_output_detail input[type="text"], #view_m2_exception_output_detail input[type="date"]').off('keypress').on('keypress', function(e) {
//		if (e.which === 13) {
//			performExceptionOutputDetailSearch();
//		}
//	});
...
//	return {
//		total: globalExceptionOutputDetailData.length,
//		currentPage: currentExceptionOutputDetailPage,
//		itemsPerPage: exceptionOutputDetailItemsPerPage,
//		data: globalExceptionOutputDetailData
//	};
//}
//
////📌 Memo 컬럼 자동 폭 조정 (가장 긴 텍스트 기준으로 컬럼 전체 폭 통일)
//function adjustExceptionOutputDetailMemoWidth() {
//  var $memoHeader = $('table.data-table.m2_exception_output_detail th.memoVal');
//  var $memoCells  = $('#exceptionOutputDetailTableBody td.memoVal');
//
...
//  $('#exceptionOutputDetailTableBody td.memoVal .memoText').each(function () {
...
//		contentType: "application/json",
//		success: function(data) {
//			$(".exceptionOutputDetailTotalQty").text(Number(data).toLocaleString());
//		},
//		error: function(xhr, status, error) {
...
//		   });
//			ExcelExporter.downloadExcel(data, window.exceptionOutputDetailColumns, {
...
//
////삭제
//$(document).on("click", ".btnExceptionOutputDelete", function() {	
//	if ($(".exceptionOutput_chk.status-completed:checked").length > 0) {
...
//
//// 관리자용 삭제
//$(document).on("click", ".btnAdminExceptionOutputDelete", function() {	
//	if ($(".exceptionOutput_chk.status-completed:checked").length > 0) {
//		alert(i18n.t('validation.confirm.items.delete'));
//		// 확정된 항목이 있습니다. 인퍼테이스 삭제 후 항목을 삭제할 수 있습니다.
//		return;
...
//	if ($(".exceptionOutput_chk.status-waiting:checked").length > 0) {
